...
class MultiLevelRankingLossTrainer(Trainer):
    ...
    def _compute_dual_mlr_loss(self,
        ...
        feature_s_loss = feature_s_loss * self.feature_loss_weight
        feature_c_loss = feature_c_loss * self.feature_loss_weight
        feature_o_loss = feature_o_loss * self.feature_loss_weight
        # TODO
        # feature_mlr_loss = feature_s_loss + feature_c_loss + feature_o_loss
        feature_mlr_loss = feature_o_loss
        ...
        score_s_loss = score_s_loss * self.score_loss_weight
        score_c_loss = score_c_loss * self.score_loss_weight
        score_o_loss = score_o_loss * self.score_loss_weight
        # TODO
        # score_mlr_loss = score_s_loss + score_c_loss + score_o_loss
        score_mlr_loss = score_o_loss
        bce_loss = self._compute_bce_loss(all_scores)
        bce_loss = bce_loss * self.bce_loss_weight
        # TODO
        # dual_mlr_loss = feature_mlr_loss + score_mlr_loss + bce_loss
        dual_mlr_loss = bce_loss
        loss_info_dict = {
    ...
    def _compute_separation_loss(self, all_data_points):
        ...
        if self.weighted_s_loss:
            inter_bounds = inter_bounds * inter_weights
        ...
        for i, better_centroid in enumerate(centroids):
            for worse_centroid in centroids[i+1:]:
                ...
                if order_loss is None:
                    ...
                    order_loss = order_loss + cur_order_loss
